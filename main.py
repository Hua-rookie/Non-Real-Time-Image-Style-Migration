# coding:utf-8
from stu_upload.style_transfer import *
...
def test_speed_up():
    ...
    conv = ConvolutionalLayer(3, 256, 256, 1, 1, speedup=False)
    conv.init_param()
    ...
    speedup_conv = ConvolutionalLayer(3, 256, 256, 1, 1, speedup=True)
    ...
    pooling = MaxPoolingLayer(2, 2, speedup=False)
    stamp = time.time()
    pooling_forward_result = pooling.forward(test_data)
    pooling_forward_time = time.time()-stamp
    print('pooling forward raw time: %f ms'%(pooling_forward_time*1000))
    stamp = time.time()
    pooling_backward_result = pooling.backward(test_dloss)
    pooling_backward_time = time.time()-stamp
    print('pooling backward raw time: %f ms'%(pooling_backward_time*1000))

    speedup_pooling = MaxPoolingLayer(2, 2, speedup=True)
    stamp = time.time()
    speedup_pooling_forward_result = speedup_pooling.forward(test_data)
    speedup_pooling_forward_time = time.time()-stamp
    print('pooling forward speedup time: %f ms'%(speedup_pooling_forward_time*1000))
    stamp = time.time()
    speedup_pooling_backward_result = speedup_pooling.backward(test_dloss)
    speedup_pooling_backward_time = time.time()-stamp
    print('pooling backward speedup time: %f ms'%(speedup_pooling_backward_time*1000))

    speedup_pooling_forward_mse = computeMse(pooling_forward_result.flatten(), speedup_pooling_forward_result.flatten())
    speedup_pooling_backward_mse = computeMse(pooling_backward_result.flatten(), speedup_pooling_backward_result.flatten())
    if speedup_pooling_forward_mse < 0.003 and speedup_pooling_backward_mse < 0.003:
        print('SPEEDUP POOLING TEST PASS.')
    else:
        print('SPEEDUP POOLING TEST FAILED.')
        exit()

    print('POOLING FORWARD SPEEDUP RATIO: %f'%(pooling_forward_time / speedup_pooling_forward_time))
    print('POOLING BACKWARD SPEEDUP RATIO: %f'%(pooling_backward_time / speedup_pooling_backward_time))
